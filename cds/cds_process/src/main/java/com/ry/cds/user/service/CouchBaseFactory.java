...
import com.ry.cds.springbootframe.couchbase.CouchBaseProtoBufManager;
...
public class CouchBaseFactory {
	...
	public void init() {
		...
		try {
			...
			for (CouchBaseConfig couchBaseConfigBO : couchBaseConfigs) {
				generateConfiguration(nodeList, classicAuthenticator, couchBaseConfigBO);
			}
	 ...
	 */
	private static void generateConfiguration(List<String> nodeList, ClassicAuthenticator classicAuthenticator,
			CouchBaseConfig couchBaseConfigBO) {
		try {
			String configValue = CryptogramHelper.decryptThreeDESECB(couchBaseConfigBO.getCouchBaseConfigValue(),
					CommonConst.DESKEY);
	...
	public static <T extends ICouchBaseStoredObject> boolean add(T value, long numOfMinutes) {
		try {
			return _couchBaseProtoBufManager.add(value.key(), value, numOfMinutes,
					getBucketName(value.couchbaseSection()));
		} catch (Exception e) {
			logger.error("添加couchbase缓存异常", e);
			return false;
		}
	...
	public static <T extends ICouchBaseStoredObject> boolean update(T value, long numOfMinutes) {
		try {
			return _couchBaseProtoBufManager.update(value.key(), value, numOfMinutes,
					getBucketName(value.couchbaseSection()));
		} catch (Exception e) {
			logger.error("更新couchbase缓存异常", e);
			return false;
	...
	public static <T extends ICouchBaseOperationObject & ICouchBaseStoredObject> boolean updateCas(T value,
			long numOfMinutes) {
		try {
			return _couchBaseProtoBufManager.updateCas(value.key(), value, numOfMinutes,
					getBucketName(value.couchbaseSection()));
		} catch (Exception e) {
			logger.error("更新couchbase缓存异常", e);
			return false;
