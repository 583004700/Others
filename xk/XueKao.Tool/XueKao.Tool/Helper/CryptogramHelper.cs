...
namespace XueKao.Tool.Helper
{
    ...
    public class CryptogramHelper
    {
        ...
        public static string Encrypt3DESWithTrueKey(string sourceStr, string key)
        {
            ...
            DES.Key = Encoding.UTF8.GetBytes(GetKey(key));
            DES.Mode = CipherMode.ECB;
        ...
        public static string Decrypt3DES(string sourceStr)
        {
            ...
            var str = Encoding.UTF8.GetString(DES.Key);
        ...
        public static string Decrypt3DES(string sourceStr, string key)
        {
            ...
            DES.Key = hashMD5.ComputeHash(Encoding.UTF8.GetBytes(key));
            DES.Mode = CipherMode.ECB;
            DES.Padding = PaddingMode.PKCS7;       //默认值  
        ...
        /// <returns></returns>
        public static string MD5Base64(string sourceStr, Encoding encoding)
        {
            var pBytes = encoding.GetBytes(sourceStr);
            var md5 = new MD5CryptoServiceProvider();
            var oBytes = md5.ComputeHash(pBytes);
            return (Convert.ToBase64String(oBytes));
        }
