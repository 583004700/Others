...
public class HbaseDemo {
	...
	public void init() throws Exception {
		...
		conf.set("hbase.zookeeper.quorum", "zwb107:2181,zwb108:2181,zwb109:2181");
	...
	public void testModify() throws Exception {
		...
		f2.setMinVersions(1).setMaxVersions(5);
		// 添加新的ColumnFamily
	...
	public void testFilter() throws Exception {
		 ...
		 // 行过滤器
		 /*RowFilter rf1 = new RowFilter(CompareFilter.CompareOp.LESS, new BinaryComparator(Bytes.toBytes("user002")));//行键小于user002
		 RowFilter rf2 = new RowFilter(CompareFilter.CompareOp.EQUAL, new SubstringComparator("00"));//行键包含00
		 testScan(rf1); 
		 System.out.println("**********"); 
		 testScan(rf2);*/
		...
		// 针对指定一个列的value来过滤
		/*SingleColumnValueFilter scvf = new SingleColumnValueFilter("base_info".getBytes(), "password".getBytes(), CompareFilter.CompareOp.GREATER, "12345".getBytes());
	...
	public void pageScan() throws Exception{
		...
		Table table = conn.getTable(TableName.valueOf("t_user_info"));
		Filter filter = new PageFilter(2);   // 一次需要获取一页的条数
		byte[] lastRow = null;  
		int totalRows = 0;  
		while (true) {  
		    Scan scan = new Scan();  
