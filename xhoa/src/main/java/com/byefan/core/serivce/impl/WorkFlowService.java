...
public class WorkFlowService implements IWorkFlowService {
    @Autowired//RuntimeService 执行管理，包括启动、推进、删除流程实例等操作  是activiti的流程执行服务类。可以从这个服务类中获取很多关于流程执行相关的信息。
     ...
     */
    @Override
    public String addExpense(Integer userId, String processId, Map<String, Object> param) {
        param.put("taskUser", userId);
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(processId, param);
     ...
     */
    @Override
    public ProcessInstance addExpense(String processId, Map<String, Object> param) {
     ...
     */
    @Override
    public String apply(String taskId, String nextUserId, Map<String, Object> param) {
...
//     * @param instanceId
//     */
//    protected void addCommentAndUpdateProcessStatus(String taskId, String instanceId) {
//        //兼容处理
//        Task task = taskService.createTaskQuery().taskId(taskId).singleResult();
//
//        //2.修改流程实例的状态
...
//    }
    ...
    public Map backToStep(String taskId, Map<String, Object> map) {
        ...
        if (taskEntity != null) {
            Activity distActivity = findFlowElementById(taskEntity.getProcessDefinitionId(), taskEntity.getTaskDefinitionKey());
...
//                      new ReturnVo<>(ReturnCode.FAIL, "并行节点无法驳回，请选择其他节点!");
                ...
                //6. 驳回到disk节点
                Activity currActivity = findFlowElementById(taskEntity.getProcessDefinitionId(), taskEntity.getTaskDefinitionKey());
