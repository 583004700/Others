function del(id) {
    parent.layer.confirm('确认删除？', {
        ...
        $.ajax({
            type: "post",
            url: "/reimburseD/del",    //向后端请求数据的url
            data: {id: id},
            dataType: "json",
            success: function (data) {
                if (data.code == 200) {
                    ...
                    setTimeout(function () {
                        window.location.href = "/fee/queryReimburseD";
                    }, 1000);
...
$(document).ready(function () {
    ...
    $(window).bind('resize', function () {
        ...
        $('#reimburseD_table_logs').setGridWidth(width);
    });
    $("#reimburseD_table_logs").jqGrid({
        url: '/reimburseD/listPg',
        datatype: "json",
        mtype: 'POST',
        postData: $("#queyrReimburseD").serializeJson(), //发送数据
        altRows: true,
        altclass: 'bgColor',
        height: "auto",
        page: 1,//第一页
        ...
        pager: jQuery("#reimburseD_pager_logs"),
        viewrecords: true,
        caption: "",
        add: false,
        edit: true,
        ...
        ondblClickRow: function (rowid, iRow, iCol, e) {
            ...
            edit(rowid);
            // page('/fee/editReimburseD?id=' + rowid, '角色编辑');
        },
    ...
    $("#reimburseDSearch").click(function () {
        // alert(JSON.stringify($("#role").serializeJson()));
        $("#reimburseD_table_logs").emptyGridParam() ;
        $("#reimburseD_table_logs").jqGrid('setGridParam', {
            postData: $("#queryReimburseD").serializeJson(), //发送数据
        }).trigger("reloadGrid"); //重新载入
    });

    $("#reimburseD_table_logs").setSelection(4, true);
    $("#reimburseD_table_logs").jqGrid('navGrid', '#reimburseD_pager_logs', {
        edit: false,
        add: false,
        del: false,
        search: false
    ...
    $("#addBtn").click(function () {
        document.getElementById("reimburseDForm").reset();
        // $("input").val('');
    ...
    $("#account_table_logs").jqGrid({
        url: '/account/listPgForReimburseD',
        datatype: "json",
        mtype: 'POST',
...
function submitHander(t, url) {
    if ($("#reimburseDForm").valid()) {
        // alert($("#form").serialize());
        var param = $("#reimburseDForm").serializeJson();
